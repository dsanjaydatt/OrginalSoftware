...
def getOpdPatientXray(regno):
    ...
    try:
        sql = "select p.regno,pfname,pmname,psname,sex,age,'OPD',o.opdid,agetype from patient_registration p,opdvisit o where o.regno=p.regno and o.regno='{}' order by vdate desc limit 1".format(
            regno)
        cursor.execute(sql)
...
def getWardPatientXray(regno):
    ...
    try:
        sql = "select p.regno,pfname,pmname,psname,sex,age,awb.bname,w.wrd_id,i.wardid from patient_registration p,ward_main w,ipdvisit i,admin_ward_bdname awb where w.regno=p.regno and w.wardstatus=1 and w.ipdid=i.ipdid and w.bedno=awb.bid and i.regno='{}' order by ipddate desc limit 1".format(
            regno)
        cursor.execute(sql)
        ...
        if request.method == 'POST':
            ...
            for i in range(row):
                ...
                filename = secure_filename(
                    regno+'_'+'{0:%d-%m-%Y %H:%M:%S}'.format(datetime.datetime.now())+'_'+fname+fileext)
...
def updateSubPartName():
    ...
    try:
        if request.method == 'POST':
            ...
            dbcolumn.append('amount')
            # The column name on the basis of we update record "MUST BE THE LAST ELEMENT OF dbcolumn"
            dbcolumn.append('subid')
            htmlcolumn.append(request.form['xid'])
            htmlcolumn.append(subxray)
            htmlcolumn.append(request.form['amount'])
            # The column name on the basis of we update record "MUST BE THE LAST ELEMENT OF htmlcolumn"
            htmlcolumn.append(subid)
            result = up.UpdateData(dbcolumn, htmlcolumn, tablename)
            return result
    except Exception as e:
        return str(e)
...
def getPatientDetail(regno):
    ...
    try:
        sql = "select p.regno,pfname,pmname,psname,sex,age,x.location,agetype from patient_registration p,opdvisit o,xray x where p.regno=o.regno and o.regno=x.regno and x.regno='{}' and x.location='OPD' order by xdate desc limit 1".format(
            regno)
        cursor.execute(sql)
        return cursor.fetchall()
    except Exception as e:
...
def getWardPatientDetail(regno):
    ...
    try:
        sql = "select p.regno,pfname,pmname,psname,sex,age,awb.bname,w.wrd_id,i.wardid,x.pid from xray x, patient_registration p,ward_main w,ipdvisit i,admin_ward_bdname awb where w.regno=p.regno and w.regno=x.regno and w.wardstatus=1 and w.bedno=awb.bid and w.ipdid=i.ipdid and w.wrd_id=x.pid and x.regno='{}' and x.location!='OPD' order by xdate desc limit 1".format(
            regno)
        cursor.execute(sql)
        return cursor.fetchall()
    except Exception as e:
...
def getOpdUpdateXray(regno):
    ...
    try:
        sql = "select o.opdid,x.id,x.xdate,ax.xid,ax.xrayname,s.subid,s.subxray,x.amount,x.upload from patient_registration p,opdvisit o,xray x,admin_xname ax,admin_subname s where p.regno=o.regno and o.opdid=x.pid and ax.xid=x.xtype and x.location='OPD' and s.subid=x.stype and x.regno='{}' order by xdate desc".format(
            regno)
        cursor.execute(sql)
        print(sql)
        return cursor.fetchall()
...
def getAllXray(xraytype):
    ...
    try:
        sql = "select subid,subxray from admin_xname ax,admin_subname s where s.xid=ax.xid and s.xid='{}'".format(
            xraytype)
        cursor.execute(sql)
        return cursor.fetchall()
    except Exception as e:
...
def getAllSubXray(subxray):
    ...
    try:
        sql = "select amount from admin_subname where subid='{}'".format(
            subxray)
...
def getPatientNameDetails(regno):
    ...
    try:
        sql = "select p.regno,p.pfname,p.pmname,p.psname,p.sex,p.age,p.agetype from patient_registration p,xray x where p.regno=x.regno and x.regno='{}' order by xdate desc limit 1".format(
            regno)
        cursor.execute(sql)
        return cursor.fetchall()
    except Exception as e:
...
def getPatientXrayDetailXray(regno):
    ...
    try:
        sql = " select x.xdate,ax.xid,ax.xrayname,s.subid,s.subxray,x.amount,x.upload,x.location from patient_registration p,xray x,admin_xname ax,admin_subname s where p.regno=x.regno and ax.xid=x.xtype and s.subid=x.stype and x.regno='{}' order by xdate desc".format(
            regno)
        cursor.execute(sql)
        return cursor.fetchall()
    except Exception as e:
...
def getPatientXrayByDate(filldate):
    ...
    try:
        sql = "select p.regno,pfname,pmname,psname,p.sex,p.age,p.agetype,x.location,x.xdate,ax.xid,ax.xrayname,s.subid,s.subxray,x.amount from patient_registration p,xray x,admin_xname ax,admin_subname s where p.regno=x.regno and ax.xid=x.xtype and s.subid=x.stype and x.xdate='{}' order by xdate desc".format(
            filldate)
        cursor.execute(sql)
        return cursor.fetchall()
    except Exception as e:
...
def monthlyXrayReport(exray):
    ...
    for i in range(0, len(allxray)):
        xraynames = "count(case when xtype=" + \
            str(allxray[i][0])+" then xtype end) as "+str(allxray[i][1])
        if i == 0:
            xtotal = xtotal + \
                "count(case when xtype="+str(allxray[i][0])+" then xtype end)"
        else:
            xtotal = xtotal + \
                "+count(case when xtype="+str(allxray[i][0])+" then xtype end)"
    ...
    endpart = xtotal + \
        " from xray xm,admin_xname xa where  xm.xtype = xa.xid and xm.xdate between '{}' and '{}' group by month(xm.xdate)"
...
def getAllXrayData(Fdate, Tdate):
    try:
        sql = " select p.regno,pfname,pmname,psname,sex,age,agetype,x.location,x.xdate,ax.xid,ax.xrayname,s.subid,s.subxray,x.amount from patient_registration p,xray x,admin_xname ax,admin_subname s where p.regno=x.regno and ax.xid=x.xtype and s.subid=x.stype and x.xdate between '{}' and '{}'".format(
            Fdate, Tdate)
        cursor.execute(sql)
        return cursor.fetchall()
    except Exception as e:
