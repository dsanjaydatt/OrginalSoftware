...
def ShowAllDrugName(ViewAvailDrug):
    try:
        ...
        sql = "select drugname,medicine_type,min_value,max_value from main_medicine where medicine_type='{}'".format(
            drugtype1)
        print(sql)
...
def UpdateMinMaxValue():
    try:
        ...
        sql = "Select min_value,max_value from main_medicine where drugname='{}' and medicine_type='{}'".format(
            dname, dtype)
        cursor.execute(sql)
        return cursor.fetchall()
    except Exception as e:
...
def GetAlldata():
    ...
    try:
        if len(drugname) >= 1:
            sql = "select drugname,medicine_type,min_value,max_value,med_id from main_medicine where drugname like '{}%'".format(
                drugname)
            cursor.execute(sql)
            outdata = cursor.fetchall()
            for i in range(0, len(outdata)):
                med = outdata[i][0] + " >  " + outdata[i][1] + "  > MinValue: " + outdata[i][2] + \
                    " > MaxValue: " + outdata[i][3] + \
                    "  > medid: " + str(outdata[i][4])
                arr_rows.append(med)
...
def UpdateDrugNameDrugType():
    ...
    try:
        if request.method == 'POST':
            ...
            htmlcolumn.append(request.form['min_value'])
            htmlcolumn.append(request.form['max_value'])
            # The column name on the basis of we update record "MUST BE THE LAST ELEMENT OF htmlcolum
            htmlcolumn.append(request.form['medicineid'])
...
def ShowInventoryUpadte():
    try:
        if request.method == 'POST':
            sql = "select i.inv_id,dt.dist_type_id,dt.dist_type_name,d.distdet_id,d.distributor_name,m.med_id,m.drugname,m.medicine_type,i.hsn_code,i.quantity,i.rate,i.batch_no,i.expiry_date,i.manufacturing_date,i.discount,i.free_med,i.mrp,i.pack,i.taxable_amt,i.cgst,i.sgst,i.amount,i.rack_no,i.drugdept_type,i.remark,i.date,i.unitprice,i.totalqty from inventory_detail i,main_medicine m ,distributor_detail d,distributor_type_detail dt where i.distdet_id=d.distdet_id and d.dist_type_id = dt.dist_type_id and m.med_id=i.med_id and inv_id='{}'".format(request.form[
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         'inventoryid'])
            cursor.execute(sql)
    ...
    try:
        if request.method == 'POST':
            ...
            if result == 1:
                ...
                # Has Multiple Value
                row = len(request.form.getlist('inve_id'))
                print("LENGHT", row)
...
def ViewUpdateOutwardDate():
    ...
    try:
        ...
        filters = "and od.issued_date between '{}' and '{}'".format(
            idateto, idatefr)
...
def ViewUpdateOutwardWard():
    ...
    try:
        fields = "select od.outw_id,od.issued_date,od.time,m.drugname,m.medicine_type,i.batch_no,i.expiry_date,i.manufacturing_date,w.wname,md.fix_issued_qty,od.person_name,md.meddet_id"
        joins = "from inventory_detail i,main_medicine m,admin_wardname w,outward_detail od,medicine_outward_detail md where m.med_id=i.med_id and i.inv_id=md.inv_id and md.outw_id = od.outw_id and od.ward_id=w.wid"
        filters = " and od.issued_date between '{}' and '{}' and w.wid='{}'".format(
            idateto, idatefr, wname)
        sql = fields+" "+joins+" "+filters
        print("ward", sql)
        cursor.execute(sql)
        return cursor.fetchall()
...
def ViewUpdateOutwardDrugname():
    ...
    try:
        fields = "select od.outw_id,od.issued_date,od.time,m.drugname,m.medicine_type,i.batch_no,i.expiry_date,i.manufacturing_date,w.wname,md.fix_issued_qty,od.person_name,md.meddet_id"
        joins = "from inventory_detail i,main_medicine m,admin_wardname w,outward_detail od,medicine_outward_detail md where m.med_id=i.med_id and i.inv_id=md.inv_id and md.outw_id = od.outw_id and od.ward_id=w.wid"
        filters = "and m.drugname='{}' and m.medicine_type='{}'".format(
            drugname, drugtype)
        sql = fields+" "+joins+" "+filters
        print("drugnam", sql)
        cursor.execute(sql)
...
def getViewUpdateOutward():
    try:
        ...
        joins = "from inventory_detail i,main_medicine m,admin_wardname w,outward_detail od,medicine_outward_detail md where m.med_id=i.med_id and i.inv_id=md.inv_id and md.outw_id = od.outw_id and od.ward_id=w.wid"
        filters = "and od.outw_id='{}' and md.meddet_id='{}'".format(
            outw_id, meddet_id)
        sql = fields+" "+joins+" "+filters
        print("ddete", sql)
        cursor.execute(sql)
...
def ShowByDrugName(viewdrugname):
    try:
        ...
        drugtype = drugtype[2:-1]
        print("HELLO", drugname, drugtype)
        sql = "Select od.outw_id, m.drugname, m.medicine_type, i.drugdept_type, a.wname, i.batch_no, i.pack, i.totalqty, i.mrp,i.unitprice, DATE_FORMAT(i.expiry_date,'%d-%m-%y'), i.amount, i.rack_no,md.issued_qty from inventory_detail i, main_medicine m , admin_wardname a, outward_detail od, medicine_outward_detail md  where od.outw_id=md.outw_id and od.ward_id=a.wid and md.inv_id=i.inv_id and i.med_id=m.med_id and m.drugname like '%{}%' and m.medicine_type='{}' and od.issued_date between '{}' and'{}'".format(
            drugname, drugtype, issuedfrom, issuedto)
        print(sql)
        cursor.execute(sql)
        a = cursor.fetchall()
...
def ShowByDrugType(viewdrugtype):
    try:
        ...
        sql = "select od.outw_id  m.drugname, m.medicine_type, i.drugdept_type, a.wname,i.batch_no, i.pack, i.totalqty, i.mrp, i.unitprice, DATE_FORMAT(i.expiry_date,'%d-%m-%y'), i.amount, i.rack_no,md.issued_qty from inventory_detail i,main_medicine m ,admin_wardname a,outward_detail od,medicine_outward_detail md  where od.outw_id=md.outw_id and od.ward_id=a.wid and md.inv_id=i.inv_id and i.med_id=m.med_id and m.medicine_type='{}' and od.issued_date between '{}' and'{}'".format(
            drugtype, issuedfrom, issuedto)
        cursor.execute(sql)
        a = cursor.fetchall()
...
def ShowWardIssuedDate(viewbywardissueddate):
    try:
        ...
        issuedto = request.form['issuedto']
        viewbywardissueddate = viewbywardissueddate + \
            "OutwardViewWardByissuedDateList.xlsx"
        sql = "select od.outw_id, m.drugname,m.medicine_type,i.drugdept_type,a.wname,i.batch_no,i.pack,i.totalqty,i.mrp,i.unitprice,DATE_FORMAT(i.expiry_date,'%d-%m-%y'),i.amount,i.rack_no,md.issued_qty from inventory_detail i,main_medicine m ,admin_wardname a,outward_detail od,medicine_outward_detail md where od.outw_id=md.outw_id and od.ward_id=a.wid and md.inv_id=i.inv_id and i.med_id=m.med_id and od.issued_date between '{}' and '{}'and a.wid='{}'".format(
            issueddatefrom, issueddateto, issuedto)
        print(sql)
        cursor.execute(sql)
        a = cursor.fetchall()
...
def ShowByEntryDate(entrydatesearch):
    try:
        ...
        sql = "Select i.inv_id,m.drugname,m.medicine_type,i.drugdept_type,i.batch_no,i.hsn_code,i.pack,i.quantity,i.rate,i.mrp,i.unitprice,DATE_FORMAT(i.expiry_date, '%d-%m-%Y'),DATE_FORMAT(manufacturing_date,'%d-%m-%Y'),i.discount,i.free_med,i.taxable_amt,i.cgst,i.sgst,i.amount,i.rack_no,i.remark,i.temp_quantity,d.distributor_name from inventory_detail i,main_medicine m,distributor_detail d where d.distdet_id=i.distdet_id and m.med_id=i.med_id and i.date between '{}' and '{}' order by(i.expiry_date)".format(
...
def ShowByExpiry(expiry):
    try:
        ...
        sql = "Select i.inv_id,m.drugname,m.medicine_type,i.drugdept_type,i.batch_no,i.hsn_code,i.pack,i.quantity,i.rate,i.mrp,i.unitprice,DATE_FORMAT(i.expiry_date, '%d-%m-%Y'),DATE_FORMAT(manufacturing_date,'%d-%m-%Y'),i.discount,i.free_med,i.taxable_amt,i.cgst,i.sgst,i.amount,i.rack_no,i.remark,i.temp_quantity from inventory_detail i,main_medicine m where m.med_id=i.med_id and i.expiry_date between '{}' and '{}' order by(expiry_date)".format(
            expiryfrom, expiryto)
        cursor.execute(sql)
        b = cursor.fetchall()
...
def ShowByDeptNameExpiry(Deptname_xpiry):
    try:
        ...
        sql = "Select i.inv_id,m.drugname,m.medicine_type,i.drugdept_type,i.batch_no,i.hsn_code,i.pack,i.quantity,i.rate,i.mrp,i.unitprice,DATE_FORMAT(i.expiry_date, '%d-%m-%Y'),DATE_FORMAT(manufacturing_date,'%d-%m-%Y'),i.discount,i.free_med,i.taxable_amt,i.cgst,i.sgst,i.amount,i.rack_no,i.remark,i.temp_quantity from inventory_detail i,main_medicine m where m.med_id=i.med_id and i.expiry_date between '{}' and '{}' and i.drugdept_type='{}'order by(expiry_date)".format(
            expiryFrom, expiryTo, deptname)
...
def UpdateOutwardMedicineDetail():
    ...
    try:
        if request.method == 'POST':
            ...
            # The column name on the basis of we update record "MUST BE THE LAST ELEMENT OF htmlcolum
            htmlcolumn.append(request.form['outwardid'])
            result = up.UpdateData(dbcolumn, htmlcolumn, tablename)
...
def GetAll_DistributorName(dist_type_id):
    try:
        sql = "select distdet_id,distributor_name from distributor_detail where dist_type_id='{}'".format(
            dist_type_id)
        cursor.execute(sql)
        return cursor.fetchall()
    except Exception as e:
...
def getDistributorAllData(distributor_id):
    try:
        sql = "select dn.*,dt.dist_type_name from distributor_detail dn,distributor_type_detail dt where dn.dist_type_id = dt.dist_type_id AND deletestatus='0' AND dn.distdet_id='{}'".format(
            distributor_id)
        cursor.execute(sql)
        return cursor.fetchall()
    except Exception as e:
...
def getDrugname():
    ...
    arr_rows = []
    if len(hint) >= 1:
        sql = "select drugname,UPPER(medicine_type) from main_medicine where drugname LIKE '{}%'".format(
            hint)
        print(sql)
        cursor.execute(sql)
        dt = cursor.fetchall()
...
def getDrugTypeName(drugtype):
    ...
    drugtype = drugtype[2:-1]
    try:
        sql = "select med_id,drugname,UPPER(medicine_type) from main_medicine where drugname='{}' AND medicine_type='{}' ".format(
            drugname, drugtype)
        cursor.execute(sql)
...
def UpdateDistributor():
    ...
    try:
        if request.method == 'POST':
            ...
            htmlcolumn.append(request.form['distributor_id'])
...
def getAllMedicineNoFilter():
    ...
    try:
        if len(drugname) >= 1:
            sql = "Select i.inv_id,m.drugname,m.medicine_type,i.batch_no,DATE_FORMAT(i.expiry_date, '%d/%m/%Y'),DATE_FORMAT(i.manufacturing_date, '%d/%m/%Y'),i.temp_quantity from inventory_detail i,main_medicine m where i.med_id=m.med_id and m.drugname  like '{}%'".format(
                drugname)
            print(sql)
            cursor.execute(sql)
            outdata = cursor.fetchall()
            for i in range(0, len(outdata)):
                med = str(outdata[i][0]) + " >  " + outdata[i][1] + " > [" + outdata[i][2] + "] >  Batch No: " + \
                    outdata[i][3] + " > Expiry Date: " + \
                    outdata[i][4] + " > MFG Date: " + outdata[i][5]
                arr_rows.append(med)
            print("MYDATA   ", arr_rows)
            return arr_rows
        else:
...
def RetrieveData():
    ...
    try:
        if len(drugname) >= 1:
            ...
            for i in range(0, len(outdata)):
                med = outdata[i][0] + " > [" + outdata[i][1] + "] >Issuedqty: " + outdata[i][2] + " > Batchno: " + \
                    outdata[i][3] + ">invid:" + str(outdata[i][4])+">tempqty:"+str(
                        outdata[i][5])+">meddetid:"+str(outdata[i][6])
                #print("hi there",med)
                arr_rows.append(med)
                # print(arr_rows)
            return arr_rows
...
def MedicineReturnByDateSearch(returndate):
    ...
    try:
        sql = "select mr.medret_id,m.drugname,m.medicine_type,i.batch_no,mr.person_name,a.wname,mr.return_qty from inventory_detail i,main_medicine m,medicine_return mr,admin_wardname a where mr.inv_id=i.inv_id and mr.wardname=a.wid and i.med_id=m.med_id and  mr.date between '{}' and '{}'".format(
...
def MedicineReturnByWardDateSearch(returnward):
    ...
    try:
        sql = "select mr.medret_id,m.drugname,m.medicine_type,i.batch_no,mr.person_name,a.wname,mr.return_qty from inventory_detail i,main_medicine m,medicine_return mr,admin_wardname a where mr.inv_id=i.inv_id and mr.wardname=a.wid and i.med_id=m.med_id and mr.wardname ='{}' and mr.date between '{}' and '{}'".format(
            wardname, returndatefrom, returndateto)
        print(sql)
        cursor.execute(sql)
        a = cursor.fetchall()
...
def showDetailsByGeneralSearchMedicineReturn():
    ...
    try:
        sql = "select mr.medret_id,m.drugname,m.medicine_type,i.batch_no,mr.person_name,a.wname,mr.return_qty from inventory_detail i,main_medicine m,medicine_return mr,admin_wardname a where mr.inv_id=i.inv_id and mr.wardname=a.wid and i.med_id=m.med_id and  mr.medret_id='{}'".format(
            medret_id)
        cursor.execute(sql)
        return cursor.fetchall()
    except Exception as e:
            ...
            for i in range(len(expdata)):
                ...
                if i <= len(noti):
                    if expdata[i][0] != noti[i][1]:
                        ...
                        htmlcolumn.append(str(expdata[i][0]))
                        htmlcolumn.append("Expiry")
                        result = ins.InsertData(
                            dbcolumn, htmlcolumn, tablename)
                        a = a+1
                else:
                    htmlcolumn = []
                    dbcolumn = []
...
def getDataNotificationPageExpiry(NotPageExpiry):
    try:
        ...
        for i in range(0, len(allwardname)):
            ...
            wardnames = "case when wid=" + \
                str(allwardname[i][0])+" then md.issued_qty else '0' end "
            if i == 0:
                xtotal = xtotal + "case when wid=" + \
                    str(allwardname[i][0])+" then md.issued_qty else '0' end"
            else:
                xtotal = xtotal + "+case when wid=" + \
                    str(allwardname[i][0])+" then md.issued_qty else '0' end"
